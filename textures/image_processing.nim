...
proc main =
  ...
  while not windowShouldClose(): # Detect window close button or ESC key
    ...
    for i in 0..<NumProcesses:
      ...
      drawText(processText[i], int32(toggleRecs[i].x + toggleRecs[i].width / 2 - measureText(
          processText[i], 10) / 2), toggleRecs[i].y.int32 + 11, 10, if
          i == currentProcess.ord or i == mouseHoverRec: DarkBlue else: DarkGray)

...
proc main =
  ...
  while not windowShouldClose(): # Detect window close button or ESC key
    ...
    drawTexture(target.texture, Rectangle(x: 0, y: 0, width: target.texture.width.float32,
        ...
        gameScreenWidth*scale))*0.5'f32, y: (getScreenHeight() - (gameScreenHeight*scale))*0.5'f32,
        width: gameScreenWidth*scale, height: gameScreenHeight*scale), Vector2(x: 0, y: 0), 0, White)
    endDrawing()
    # ------------------------------------------------------------------------------------

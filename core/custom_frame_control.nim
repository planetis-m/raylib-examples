...
proc main =
  ...
  while not windowShouldClose(): # Detect window close button or ESC key
    ...
    drawText(cstring(&"{timeCounter * 1000'f32:03.0f} ms"), position.int32 - 40,
             getScreenHeight() div 2 - 100, 20, Maroon)
    drawText(cstring(&"PosX: {position:03.0f}"), position.int32 - 50,
             getScreenHeight() div 2 + 40, 20, Black)
    drawText("Circle is moving at a constant 200 pixels/sec,\nindependently of the frame rate.",
             10, 10, 20, DarkGray)
    drawText("PRESS SPACE to PAUSE MOVEMENT", 10, getScreenHeight() - 60, 20, Gray)
    drawText("PRESS UP | DOWN to CHANGE TARGET FPS", 10, getScreenHeight() - 30, 20, Gray)
    drawText(cstring(&"TARGET FPS: {targetFPS}"), getScreenWidth() - 220, 10, 20, Lime)
    drawText(cstring(&"CURRENT FPS: {int32(1'f32 / deltaTime)}"), getScreenWidth() - 220, 40, 20, Green)
    endDrawing()
